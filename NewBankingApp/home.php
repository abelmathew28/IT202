...
$sql = "SELECT account_activity.* , recip.`account_number` as recipent,depo.`account_number` as depositor FROM account_activity ,holders_detail recip,holders_detail depo WHERE account_activity.`depositer_detail_id` = depo.`detail_id` and `recipient_detail_id` = recip.`detail_id` and ( account_activity.`depositer_detail_id`= $userdetailsID or account_activity.`recipient_detail_id`=$userdetailsID) order by activity_id ASC
limit 10;  ";
...
.transfer {
  ...
  <body>
   ...
   <?php include 'usermenu.php'; ?>
    ...
    <div class="container">
      <h1>
        Hello  <?php echo ucfirst($_SESSION['user_name']); ?> Here are your last 10 transactions.
      </h1>
      <table>
  <tr>
